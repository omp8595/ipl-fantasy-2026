  ...
  return (
    <div>
      {entries.map((entry, idx) => {
        const isYou = entry.userId === user?.uid;
        return (
          <div key={entry.userId} style={{ ...s.row, ...(isYou ? s.rowYou : {}) }} onClick={() => viewTeam(entry)}>
            <div style={{ ...s.rank, ...(idx < 3 ? s.rankTop : {}) }}>#{idx + 1}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 500 }}>{entry.displayName}</div>
            </div>
            <div style={{ fontSize: 14, fontWeight: 500 }}>{entry.totalScore} pts</div>
          </div>
        );
      })}
      {selectedUser && (
        <div style={s.overlay} onClick={() => setSelectedUser(null)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
              <div style={{ fontSize: 15, fontWeight: 500 }}>{selectedUser.displayName} Team</div>
              <button onClick={() => setSelectedUser(null)}>Close</button>
            </div>
            {selectedUser.breakdown && Object.keys(selectedUser.breakdown).length > 0 ? (
              Object.entries(selectedUser.breakdown).map(([pid, data]) => (
                <div key={pid} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '0.5px solid #f0f0f0', fontSize: 13 }}>
                  <span>{pid.split('_').slice(1).join(' ')} {data.role==='captain'?'(C)':data.role==='viceCaptain'?'(VC)':''}</span>
                  <span>{data.finalPts || 0} pts</span>
                </div>
              ))
            ) : <div style={{ color: '#888', textAlign: 'center', padding: '1rem' }}>No team data yet</div>}
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', fontWeight: 600, borderTop: '1px solid #eee' }}>
              <span>Total</span><span style={{ color: '#FF6B00' }}>{selectedUser.totalScore} pts</span>
            </div>
          </div>
        </div>
      )}
