...
export default function SelectTeamPage() {
  ...
  const allPlayers=(match?.teams??[]).flatMap(t=>((squadsLoaded?squads[t]:SQUADS[t])||[]).map(p=>({...p,team:t,id:`${t}_${p.n.replace(/\s+/g,"_")}`})));
  const budgetUsed=selected.reduce((s,id)=>{const p=allPlayers.find(x=>x.id===id);return s+(p?.c||0);},0);
